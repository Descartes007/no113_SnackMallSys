...
import com.wen.config.OssConfiguration;
...
public class AliyunOssUtil {
    ...
    public String upload(String folderName, File file) {
        ...
        try {
            ...
            //设置文件路径和名称
            String fileName = folderName + "/" + UUID.randomUUID().toString().replace("-", "") + "-" + file.getName();
            PutObjectResult result = client.putObject(new PutObjectRequest(bucketName, fileName, file));
            client.setBucketAcl(bucketName, CannedAccessControlList.PublicRead);
            //文件上传成功后,返回当前文件的路径
            if (result != null) {
                return config.getFileHost() + fileName;
            }
        } catch (Exception e) {
