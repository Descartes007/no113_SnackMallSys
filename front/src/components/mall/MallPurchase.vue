  ...
  <div class="mall-purchase">
    <div class="product-box">
      <div class="nav-bar">
        <div v-if="productInfo" class="container">
          ...
          <el-link type="danger" @click="toReview">零食评价</el-link>
        </div>
      </div>
    </div>
    <div class="product-info">
      <div class="product-info-content container">
        <div class="img">
          <el-image
            :src="productInfo.productUrl"
            style="height: 85%; width: 70%; margin: 15%"
          ></el-image>
        </div>
        <div class="product-con">
          ...
          <div class="price-info">
            ...
            <span v-else>零食库存 {{ productInfo.productStock }}</span>
          </div>
          <div class="line"></div>
          <div class="address-box container">
            ...
            <div class="address-alter" @click="addressFormVisible = true">
              修改
            </div>
          </div>
          <div
            class="option-box"
            v-if="this.productSpec !== null && this.productSpec.length !== 0"
          >
            <div class="title">选择规格</div>
            <el-radio-group
              class="option-list"
              v-for="(option, index) in this.productSpec"
              fill="#ff6700"
              v-model="order.productSpecs"
              :key="index"
            >
    ...
    <el-dialog title="订单信息" :visible.sync="orderFormVisible" width="600px">
      ...
      >
        ...
        <el-form-item v-if="order.productSpecs" label="零食规格">
        ...
        <el-form-item v-if="isVip" label="会员折扣" style="position: relative">
          <el-input
            v-model="order.discountPrice"
            class="discount-price"
            disabled
          ></el-input>
          <el-button plain class="vip-info"
            >VIP {{ discountStrength }}折</el-button
          >
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitOrder('order')"
          >付 款</el-button
        >
...
export default {
  ...
  methods: {
    ...
    buy() {
      ...
      if (
        this.order.contactWay !== null &&
        this.order.contactWay.length === 0
      ) {
        if (
      ...
      this.$http
        ...
        .then((rep) => {
          if (rep.data.code === 200) {
            ...
            this.order.productSpecs = this.productSpec[0]; //默认选择第一个零食规格
          } else {
            this.$msg.error(rep.data.message);
          }
        })
