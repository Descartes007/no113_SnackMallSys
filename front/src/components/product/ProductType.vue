  ...
  <div class="product-type">
    <el-card class="product-hd" shadow="never">
      ...
      <el-form :model="productTypeForm" ref="productTypeForm" inline class="query-form">
        <el-form-item label="输入搜索：" prop="key">
          <el-input v-model="productTypeForm.key" @keyup.enter.native="queryProductType" clearable placeholder="品牌名称/关键字"></el-input>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card class="product-box" shadow="never">
      ...
      <el-table :data="productType" class="type-table" height="425"  border style="width: 100%">
        ...
        <el-table-column prop="typeName" label="品牌名称" width="150"></el-table-column>
        <el-table-column prop="typeDescribe" label="品牌描述" width="400"></el-table-column>
    ...
    <el-dialog title="编辑零食分类信息" class="chang-product" :close-on-click-modal="false" :visible.sync="dialogFormVisible" center>
      <el-form class="info-form" inline :rules="alterRules" :model="alterForm" ref='changeFrom'>
        <el-form-item label="品牌名称：" prop="typeName" style="width: 100%">
          <el-input size="small" v-model="alterForm.typeName" placeholder="品牌名称"></el-input>
        </el-form-item>
        <el-form-item label="品牌描述：" prop="typeDescribe" style="width: 100%">
          <el-input size="small" type="textarea" autosize v-model="alterForm.typeDescribe" placeholder="品牌描述"></el-input>
        </el-form-item>
  ...
  export default {
    ...
    data(){
      return{
        ...
        alterRules:{
          typeName: [{ required: true, message: '请输入品牌名称', trigger: 'blur' },
            { min: 1, max: 30, message: '名称长度不允许超过 30 个字符', trigger: 'blur' }],
          typeDescribe: [{ required: true, message: '请输入品牌描述', trigger: 'blur' },
      ...
      deleteCheck(index, rows){
        this.$http.post('/product/existsType?productType='+this.productType[index].typeName).then((rep)=>{
          if(rep.data.code===200){
            ...
            if(rep.data.data){
              this.$alert('类别('+name+')\t存在零食，无法删除', '警告', {confirmButtonText: '确定', type:"warning"});
            }else{
