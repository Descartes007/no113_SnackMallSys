<template>
  <div class="shopping-cart">
    <el-button
      v-if="this.tableData && tableData.length !== 0"
      class="clear-cart"
      type="primary"
        ...
        >
          ...
          <el-table-column label="零食名称">
            <template slot-scope="scope">
              <router-link
                :to="'/MallPurchase?id=' + scope.row.productId"
                style="text-decoration: none"
              >
          ...
          <el-table-column label="数量" width="180">
            <template slot-scope="scope">
              <el-input-number
                ...
                @change="calculationPrice(scope.$index)"
    ...
    <el-dialog title="订单信息" :visible.sync="orderFormVisible" width="600px">
      ...
      >
        ...
        <el-form-item label="预计支付">
          <el-input :value="baseInfo.payMoney + ' 元'" readonly></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="addCartOrder('order')"
          >付 款</el-button
        >
...
export default {
  ...
  methods: {
    ...
    buy() {
      ...
      if (
        this.baseInfo.userName !== null &&
        this.baseInfo.userName.length === 0
      ) {
        this.baseInfo.userName = this.$store.state.user["userName"];
      }
      if (
        this.baseInfo.contactWay !== null &&
        this.baseInfo.contactWay.length === 0
      ) {
        if (
          this.$store.state.user["telephone"] === null ||
          this.$store.state.user["telephone"].length === 0
        ) {
          this.$confirm(
            "检测到您未绑定手机号，快递小哥哥将无法找到您",
            "提示",
            {
    ...
    addCartOrder() {
      ...
      for (
        let index = 0, len = this.selectedData.length;
        index < len;
        index++
      ) {
        let order = {
      ...
      this.$http
        .post("/order/cartOrder", this.$qs.stringify(send))
        .then((rep) => {
          loading.close();
          if (rep.data.code === 200) {
            ...
            // 设置支付参数
            this.paymentOrderNo = send.orderNo;
            this.paymentOrderName =
              this.getCurrentDate() +
              "商城-" +
              this.baseInfo.userName +
              "购物车支付订单";
            this.paymentAmount = this.baseInfo.payMoney;

            let msg =
              "共" + productNo.length + "零食购买成功，需支付 " + money + " 元";
            this.$alert(msg, "下单成功", {
    ...
    calculationPrice(index) {
      this.tableData[index].totalPrice = (
        this.tableData[index].payAmount *
        Math.floor(
          (this.tableData[index].productPrice *
            this.tableData[index].discount) /
            100
        )
      ).toFixed(2);
      this.summation = 0;
    ...
    deleteProduct(index) {
      this.$confirm("此零食将从购物车中移除?", "提示", {
        ...
        .then(() => {
          ...
          this.$http
            .post(
              "/shoppingCart/deleteById?cartId=" + this.tableData[index].cartId
            )
            .then((rep) => {
              loading.close();
              if (rep.data.code === 200) {
    ...
    clearCart() {
      this.$confirm("是否清空购物车？", "提示", {
        ...
        .then(() => {
          ...
          this.$http
            .post(
              "/shoppingCart/deleteByUser?accountNumber=" +
          ...
          if (rep.data.code === 200) {
            ...
            for (let i = 0, len = this.tableData.length; i < len; i++) {
              if (isVip) {
                ...
                if (
                  this.tableData[i].productPrice >= 1000 &&
                  this.tableData[i].productPrice < 5000
                ) {
              ...
              this.tableData[i].totalPrice = (
                this.tableData[i].payAmount *
                Math.floor(
                  (this.tableData[i].productPrice *
...
.shopping-cart .page-main .empty-cart-top {
  ...
  padding-left: 50%;
  margin: 65px 0 130px;
  background: url(https://spring-oss.oss-cn-beijing.aliyuncs.com/images/img/cart-empty.png)
    no-repeat 30% 0;
  color: #b0b0b0;
  overflow: hidden;
  text-align: left;
