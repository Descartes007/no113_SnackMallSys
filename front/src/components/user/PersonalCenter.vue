  ...
  <div class="person-center">
      ...
      <el-card class="box-card box-abs" shadow="hover">
        ...
        <div style="position: absolute; width: 100%; top: 48%">
          ...
          <el-avatar
            ...
            :size="140"
            @error="errorHandler"
            :src="this.userForm.avatarUrl"
          >
            <img src="../../assets/img/微笑.jpg" alt="默认头像" />
          </el-avatar>
          <!--昵称 和 身份-->
          <div
            style="
          ...
          >
            <!-- <el-button
              ...
              style="
                ...
                font-weight: 700;
                color: #000;
                background-color: rgb(225, 190, 146);
              "
              @click="vipVisible = true"
              v-if="!isVip && isCustomer"
              >开通VIP
        ...
        >
          <el-form-item class="el-user-item" style="position: relative">
            <!-- <el-image
              ...
              alt="vip"
              style="position: absolute; top: -21px; right: 22px; width: 35px"
            ></el-image> -->
            <el-avatar
              :size="80"
              :src="this.userForm.avatarUrl"
          ...
          >
            <el-input
              ...
              placeholder="地址"
              readonly
              style="width: 206px; position: absolute"
            ></el-input>
            <el-button
              size="mini"
              icon="el-icon-edit"
              style="position: absolute; right: -37px; margin: 10px 0"
              circle
    ...
    >
      ...
      <div style="position: relative">
        ...
        <el-button
          ...
          size="small"
          :disabled="!show"
          style="position: absolute; right: 3px; top: 0; margin: 4px 0"
          @click="sendEmailCode"
        >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="bindingPhone" :disabled="isEditPhone"
          >确 定</el-button
        >
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button v-if="this.userForm.telephone" @click="unbindPhone"
          >解 绑</el-button
        >
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitAddress('addressForm')"
          >确 定</el-button
        >
      </div>
    </el-dialog>
    ...
    <el-dialog
      ...
      :show-close="isVip || !isCustomer"
      top="0"
      :modal="false"
      v-if="isCustomer"
    >
      <span v-if="isVip" style="color: #f8c4a2"
        >{{ getDate(vip["overdueTime"]) }} 到期</span
      >
      ...
      <div class="vip-box">
        ...
        >
          <h3>折扣卡</h3>
          <div>
            <span style="font-size: xx-large; color: #f8313c">{{
              item["discount"]
            }}</span>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button v-if="!isVip" type="primary" @click="openVip"
          >开通</el-button
        >
    ...
    <PaymentDialog
      ...
      :serial-number="paymentSerialNumber"
      :is-vip="true"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      vip: { overdueTime: "" },
      vipInfo: [
        { discount: 98, desc: "全场零食享受" },
        { discount: 95, desc: "满1000可用" },
        { discount: 90, desc: "满5000可用" },
      ],
  ...
  created() {
    ...
    if (this.isVip) {
      this.$http
        .post(
          "/vip/findVipByKey?accountNumber=" +
            this.$store.state.user["accountNumber"]
        )
        .then((rep) => {
          if (rep.data.code === 200) {
  ...
  methods: {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http
            .post(
              "/allow/update",
              this.$qs.stringify(this.userForm, { skipNulls: true })
            )
            .then((res) => {
    ...
    uploadSuccess(response) {
      if (response.code === 200) {
        try {
            ...
            .then((res) => {
              if (res.data.code === 200) {
                ...
                setTimeout(() => {
                  ...
                  backgroundImg.src = response.data;
                }, 200);
              } else {
                this.$msg.warning(res.data.message || "背景图片保存失败");
              }
            })
            .catch((err) => {
              console.error("背景上传后端保存失败：", err);
          ...
          console.error("背景上传处理异常：", error);
          this.$msg.error("背景上传处理异常");
          this.$refs["backGroundUpload"].clearFiles();
        }
      } else {
        this.$msg.error("背景上传失败");
    ...
    sendEmailCode() {
      ...
      this.$http
        .post(
          "/allow/sendHtmlCode?email=" + this.$store.state.user["accountNumber"]
        )
        .then((res) => {
          if (res.data.code === 200) {
            this.$msg.success({
    ...
    editPhoneCode() {
      ...
      if (regex.test(this.phoneInfo)) {
        this.$http
          .post("/allow/existPhone?telephone=" + this.phoneInfo.trim())
          .then((res) => {
            if (res.data.code === 200) {
              if (res.data.data === false) {
                //data 为false  代表手机号未被查询到，所以未被绑定
                this.getCode();
                this.$http
                  .post("/util/smsCode?phone=" + this.phoneInfo.trim())
                  .then((res) => {
                    if (res.data.code === 200) {
                      //绑定手机号的确定按钮可用
                      this.$msg.success({
    ...
    getDateNow() {
      ...
      this.order.orderNo =
        "Vip" + year + mouth + day + hours + random + seconds;
    },
    pay() {
      ...
      this.$http
        .post("/order/add", this.$qs.stringify(this.order, { skipNulls: true }))
        .then((rep) => {
          loading.close();
          if (rep.data.code === 200) {
            // 生成VIP序列号
            ...
            // 设置支付参数
            this.paymentOrderNo = this.order.orderNo;
            this.paymentOrderName = "商城-开通Vip支付订单";
            this.paymentAmount = this.order.payPrice;
            this.paymentSerialNumber = vipSerialNumber;
        ...
        reader.onload = (e) => {
          ...
          image.onload = () => {
            ...
            if (!isJPG) {
              ...
              // 提示用户图片比例，但仍然允许上传
              this.$msg.warning(
                "建议上传宽高比≈16:9的图片，当前图片可能在小屏幕下显示不完整"
              );
              resolve(true);
            } else {
              resolve(true);
...
.person-center .box-abs {
  height: 300px;
  width: 80%;
  min-width: 640px; /* 修改最小宽度，以适配较小的屏幕 */
  border-radius: 5px;
  text-align: left;
  top: 85px;
  position: absolute;
...
.person-center .box-abs .el-card__body,
.box-info .el-card__body {
  height: 100%;
  padding: 0;
}
