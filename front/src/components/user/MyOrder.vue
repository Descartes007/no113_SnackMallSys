  ...
  <div class="my-order">
    ...
    <!-- <div class="mall-nav">
      <el-menu :default-active="activeIndex" class="el-menu-demo" :collapse-transition="false" active-text-color="#409EFF" mode="horizontal" @select="handleSelect">
        ...
        <el-menu-item index="2" style="width: 50%">
          <el-input placeholder="请输入内容" v-model="queryProductInfo" @keyup.enter.native="queryProduct(queryProductInfo)" class="input-with-select" style="height: 45px;">
            <el-select v-model="select"  slot="prepend" placeholder="请选择" style="width: 120px;">
              <el-option label="零食名称" value="1"></el-option>
              <el-option label="零食分类" value="2"></el-option>
        ...
        <el-submenu index="3" style="width: 10%">
          <template slot="title">零食分类</template>
            ...
            <el-table-column type="expand">
              <template slot-scope="props">
                <el-form label-position="left" inline class="product-form">
                  ...
                  <el-form-item v-if="props.row.productSpecs" label="零食规格">
                    <span>{{ props.row.productSpecs }}</span>
                  </el-form-item>
                  <el-form-item label="零食类别">
                    <span>{{ props.row.productType }}</span>
                  </el-form-item>
                  <el-form-item label="零食单价">
                  ...
                  <el-form-item v-if="props.row.orderState === '待付款'">
                    <span style="color: red">{{
                      changeTime(props.row.orderTime)
                    }}</span>
                  </el-form-item>
                </el-form>
              </template>
            ...
            <el-table-column align="center" label="操作">
              <template slot-scope="scope">
                ...
                <el-button
                  ...
                  @click="receiveGoods(scope.row.orderId)"
                  >确认收货</el-button
                >
                <el-button
                  size="mini"
                ...
                <el-button
                  ...
                  type="danger"
                  slot="reference"
                  @click="cancelOrder(scope.row.orderId, scope.row.orderTime)"
                  >取消订单</el-button
                >
                <el-button
                ...
                <el-button
                  ...
                  disabled
                  type="success"
                  v-if="scope.row.orderState === '退货完成'"
                  >退货完成</el-button
                >
                <el-button
                  size="mini"
                ...
                <el-button
                  v-if="
                    scope.row.orderState === '已发货' ||
                    scope.row.orderState === '已收货' ||
                    scope.row.orderState === '已评价'
                  "
                  size="mini"
                  type="danger"
                  slot="reference"
    ...
    <div v-else class="no-order">
      <el-image
        ...
        src="https://spring-oss.oss-cn-beijing.aliyuncs.com/images/img/noorder.png"
      ></el-image>
    </div>
    <el-dialog
      title="提示"
    ...
    >
      <el-form :model="returnFrom" :rules="returnRules" ref="returnFrom">
        <el-form-item label="退货原因" prop="returnReason">
          <el-select
            v-model="returnFrom.returnReason"
            placeholder="请选择退货原因"
          >
            <el-option
              v-for="(item, index) in returnReasonName"
              :key="index"
              :label="item"
              :value="item"
            ></el-option>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="returnFormVisible = false"
          >取 消</el-button
        >
        <el-button
          size="small"
          type="primary"
          @click="submitReturnInfo('returnFrom')"
          >确 定</el-button
        >
      </div>
    </el-dialog>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          type="primary"
          style="padding: 12px 30px; margin: 0 30px"
          @click="submitInfo('changeFrom')"
          >确 定</el-button
        >
        <el-button
          style="padding: 12px 30px; margin: 0 30px"
          @click="cancelChange('changeFrom')"
          >取 消</el-button
        >
      </div>
    </el-dialog>
...
<!--      ></el-image>-->
    ...
    >
      <el-form :model="reviewFrom" :rules="reviewRules" ref="reviewFrom">
        <el-form-item label="零食名称" required prop="productName">
          <el-input
            size="small"
            v-model="reviewFrom.productName"
            readonly
            placeholder="零食名称"
          ></el-input>
        </el-form-item>
        <el-form-item
          v-if="reviewFrom.productSpecs"
          required
          label="零食规格"
          prop="productSpecs"
        >
          <el-input
            size="small"
            v-model="reviewFrom.productSpecs"
            readonly
            placeholder="零食规格"
          ></el-input>
        </el-form-item>
        ...
        >
          <el-rate
            v-model="reviewFrom.starLevel"
            :colors="colors"
            show-text
          ></el-rate>
        </el-form-item>
        <el-form-item label="零食评价" prop="productReview">
          <el-input
            type="textarea"
            style="max-height: 120px"
            :rows="3"
            placeholder="从多个角度评价宝贝，可以帮助更多想买的人"
            v-model="reviewFrom.productReview"
          ></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="cancelReview('reviewFrom')"
          >取 消</el-button
        >
        <el-button
          size="small"
          type="primary"
          @click="submitReview('reviewFrom')"
          >确 定</el-button
        >
      </div>
    </el-dialog>
    ...
    <PaymentDialog
      ...
      :order-name="paymentOrderName"
      :pay-price="paymentAmount"
      :is-vip="false"
...
import PaymentDialog from "../common/PaymentDialog"; // 导入支付弹窗组件
...
export default {
  ...
  data() {
    ...
    return {
      ...
      reviewRules: {
        ...
        productReview: [
          { required: true, message: "请填写零食评价", trigger: "blur" },
          {
            min: 1,
            max: 300,
            message: "评价内容在 1~300 个字以内",
            trigger: "blur",
          },
        ],
      },
  ...
  methods: {
    isTimeOut(time) {
      //判断未支付的订单是否超过24小时，超过则返回true, 否则返回 false
      ...
      if (this.isTimeOut(orderTime)) {
        ...
        this.$confirm("订单将被取消, 是否继续?", "提示", {
          ...
          .then(() => {
            ...
            this.$http
              .get("/order/delete?orderId=" + orderId)
              .then((res) => {
                loading.close();
    ...
    payment(orderNo, orderTime, productName, payPrice, productType) {
      if (this.isTimeOut(orderTime)) {
        ...
        this.paymentOrderName =
          "商城-" + productType + "-" + productName + "支付订单";
        this.paymentAmount = payPrice;
    ...
    applyReturn(orderId, orderNo, payPrice, orderState) {
      this.$http
        .post("/returnReason/findReasonName")
        .then((rep) => {
    ...
    submitReview(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.productReview.accountNumber =
          ...
          this.$http
            .post(
              "/productReview/add",
              this.$qs.stringify(this.productReview, { skipNulls: true })
            )
            .then((rep) => {
              loading.close();
              if (rep.data.code === 200) {
                ...
                this.$notify.error({ title: "失败", message: "零食评价失败" });
              }
            })
            .catch((err) => {
              loading.close();
    ...
    submitReturnInfo(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$http
            .post(
              "/returnGoods/addReturn",
              this.$qs.stringify(this.returnGoods, { skipNulls: true })
            )
            .then((rep) => {
              loading.close();
              if (rep.data.code === 200) {
    ...
    receiveGoods(orderId) {
      this.$confirm("请收到零食并检查无误后，再确认收货", "提示", {
        ...
        .then(() => {
          ...
          this.$http
            .post("/order/receipt?orderId=" + orderId)
            .then((rep) => {
              loading.close();
              if (rep.data.code === 200) {
                this.reqInfo();
    ...
    reqInfo() {
      ...
      this.$http
        .post(
          "/order/findOrderInfo?userAccount=" +
            this.$store.state.user["accountNumber"]
        )
        .then((rep) => {
          loading.close();
      ...
      this.$http
        .post(
          "/returnGoods/findReturnInfo?userNumber=" +
            this.$store.state.user["accountNumber"]
        )
        .then((rep) => {
          if (rep.data.code === 200) {
    ...
    delOrder(orderId) {
      if (orderId) {
        ...
        this.$http
          .post("/order/delete?orderId=" + orderId)
          .then((rep) => {
            loading.close();
            if (rep.data.code === 200) {
              this.reqInfo();
    ...
    submitInfo(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$http
            .post("/logistics/addInfo", this.$qs.stringify(this.alterForm))
            .then((rep) => {
              if (rep.data.code === 200) {
                let returnId = this.orderData[this.handleIndex].returnId;
                this.$http
                  .post("/returnGoods/sendBack?returnId=" + returnId)
                  .then((rep) => {
                    loading.close();
                    if (rep.data.code === 200) {
                      this.reqInfo();
